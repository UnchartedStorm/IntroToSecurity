...
class AES:
    ...
    def create_keys(self, key) -> None:
        # create numpy matrix
        self.key = np.array([[key[i + j] for j in range(4)] for i in range(0, 16, 4)])
        ...
        for i in range(10):
            self.round_keys.append(self.round_keys[i])
            self.round_keys[i + 1] = self.round_keys[i]

            # rotate word
            self.round_keys[i + 1] = np.roll(self.round_keys[i + 1], -1)

            # sub word
            self.sub_bytes(self.round_keys[i + 1])

            # rcon
            self.round_keys[i + 1][0][0] ^= rcon[i]
